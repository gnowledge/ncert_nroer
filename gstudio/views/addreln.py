...
def checkrt(request):
    ...
    rts=Relationtype.objects.filter(title=titl)
    if rts.count()>0:
        fl['ret']=1
    fl['slug']=slugify(titl)
    jsonobject = json.dumps(fl)
...
def addrelnform(request,meetob):
    ...
    if title:
        ...
        ob.inverse=inverse
        if slug:
            ob.slug=slugify(unidecode(slug))
        else:
            ob.slug=slugify(unidecode(title))
